...
def merge_images():
    imagewidth=cmb_width.get()
    imagespace=cmb_space.get()
    imageformat=cmb_format.get()
    ...
    # 스케치북 준비
    result_img = Image.new("RGB", (max_width, total_height+imagespace*2), (255, 255, 255))  # 배경 흰색
    y_offset = 0  # y 위치
    # for img in images:
    ...
    if (imageformat == "BMP"):
        dest_path = os.path.join(txt_dest_path.get(), "merge.bmp")
    if (imageformat == "PNG"):
        dest_path = os.path.join(txt_dest_path.get(), "merge.png")
