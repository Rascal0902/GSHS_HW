...
class Planet:
  ...
  def __init__(self, name, mass, distance, radius, color, canvas):
    ...
    self.canvas = canvas
    self.color = color
    self.angular_velocity = -math.sqrt(self.gravitational_force() / (self.mass * self.distance))
...
class Moon(Planet):
  ...
  def coordinates(self, t):
    m_theta = self.angular_position(t)
    e_theta = self.earth.angular_position(t)
    x, y = self.earth.coordinates(e_theta)
    y += self.scaled_distance * math.sin(m_theta)
    x += self.scaled_distance * math.cos(m_theta)
    return (x, y)
