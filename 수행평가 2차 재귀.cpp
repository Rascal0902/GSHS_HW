...
#define HEADER 54
using namespace std;
int ct[1025], bl[1025],Y[1025],Yc[1025],i,j,k,num,l,m=1,n,nm=1,max1=0,max2=0,Ymidx,Ymidy,Bmidx,Bmidy,P1y,P1x,P0y,P0x,P2y,P2x;
unsigned char R[HEIGHT][WIDTH],G[HEIGHT][WIDTH],B[HEIGHT][WIDTH];
double dist=0,sum=0,sum2=0,mid1,mid2;
double x1,x2,z1,z2;
FILE *fp1 = fopen("bezierbefore2.bmp","rb");
FILE *fp2 = fopen("copy21.bmp","wb");
FILE *fp3 = fopen("copy21.bmp","rb");
FILE *fp4 = fopen("copy22.bmp","wb");
...
void ctmax(int low,int high)
{
    ...
    Bmidy=bl[max2];
    Bmidx=max2;
    //printf("Bmidx=%d Bmidy=%d ",Bmidx,Bmidy);
...
void line(int low,int high)
{
    ...
    for(i=low; i<high; i++)
    {
        ...
        n=floor(double((z2-z1))*((double)(i-low)/(high-low))+z1);
        Y[i]=n;
        if(B[n][i]==225)
        {
            Yc[i]=1;
...
void makeBezier(int low,int high,int k)
{
    printf("%d %d\n",low,high);
    goBezier(low,high);
    if(k>num)
    ...
    {
       cleaner(low-1,high+1);
       half(low,high,3,HEIGHT-3);
       int mid;
       mid=(int)mid1;
       makeBezier(low,mid,k+1);
       makeBezier(mid+1,high,k+1);
    }
